...
//array of Objetcts
let taskList: { [x: string]: string }[] = [];
//view all assign tasks
let viewTasks = function (list: { [x: string]: string }[]) {
...
let newTask = function (task: string, date: string) {
  taskList.push({ task: task, taskDate: date });
  return 'Updated successfully...';
};
// Delete task function based on task date
let delTask = function (date: string, list: { [x: string]: string }[]){
  let flag=false
  list.forEach((e, index) => {
    if (e.taskDate === date) {
     delete list[index];
...
let searchTask = function (date: string, list: { [x: string]: string }[]) {
  let flag =false
  list.forEach((e, index) => {
    if (e.taskDate === date) {
      flag= true
  ...
  switch (answers.options) {
    ...
    case "Delete Task": {
      let askDate = await inquirer.prompt([
        {
          message:
            "Plz provide date of task in correct format like '2020-09-08' :",
          type: "string",
          name: "date",
        },
      ]);
